...
loop:    mult     $s0, $t1       # $s2 = row * #cols  (two-instruction sequence) 0*16
         mflo     $s2            # move multiply result from lo register to $s2 s2=0
         add      $s2, $s2, $s1  # $s2 += column counter s2=0+0=0
         sll      $s2, $s2, 2    # $s2 *= 4 (shift left 2 bits) for byte offset s2=0*4=0
         sw       $t2, data($s2) # store the value in matrix element t2=0
         addi     $t2, $t2, 1    # increment value to be stored t2=1
...
#                If we increment past last row, we're finished.
         addi     $s1, $s1, 1    # increment column counter s1=1
